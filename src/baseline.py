    ...
    def get_ref_state(self, currState, obstacleList):
        ...
        if ((distToTargetX < 5 and distToTargetY < 5)):
            ...
            if self.vehicle_state == "turn-right":
                ...
                tmp_x = 6
                tmp_y = 0
                x_offset = np.cos(target_orientation+np.pi/2) * \
                    tmp_x - np.sin(target_orientation+np.pi/2)*tmp_y
                y_offset = np.sin(target_orientation+np.pi/2) * \
                    tmp_x + np.cos(target_orientation+np.pi/2)*tmp_y
...
class VehicleController():
    ...
    def execute(self, currentPose, targetPose):
        ...
        dx = target_x - curr_x
        dy = target_y - curr_y
        xError = (target_x - curr_x) * \
            np.cos(currentEuler[2]) + (target_y -
                                       curr_y) * np.sin(currentEuler[2])
        yError = -(target_x - curr_x) * \
            np.sin(currentEuler[2]) + (target_y -
                                       curr_y) * np.cos(currentEuler[2])
